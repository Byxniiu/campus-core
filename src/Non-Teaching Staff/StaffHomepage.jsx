      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex justify-between items-center bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 shadow-2xl">
          <div className="flex items-center gap-6">
            <div className="w-16 h-16 bg-blue-950 rounded-2xl flex items-center justify-center border border-blue-900 shadow-inner overflow-hidden shadow-2xl relative group">
              {user?.avatar ? (
                <img
              ...
              <div className="absolute inset-0 bg-amber-500/0 group-hover:bg-amber-500/10 transition-colors pointer-events-none"></div>
            </div>
            <div>
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className={`flex items-center gap-3 px-6 py-3 rounded-full border text-[10px] font-black uppercase tracking-widest transition-all ${
                activeView === 'profile'
                  ? 'bg-amber-500 text-slate-950 border-amber-500 shadow-xl'
                  : 'bg-slate-800 text-slate-300 border-slate-700 hover:bg-slate-700'
              }`}
            >
              {activeView === 'profile' ? <LayoutDashboard size={14} /> : <Settings size={14} />}
              {activeView === 'profile' ? 'OPERATIONS_VIEW' : 'IDENTITY_CONFIG'}
            </button>
        ...
        {activeView === 'profile' ? (
          <div className="animate-in fade-in slide-in-from-right-10 duration-700">
            <div className="bg-slate-900/50 p-1 rounded-[3rem] border border-slate-800 shadow-2xl">
              <ProfileManager
                user={user}
                  ...
                  requests.map((req) => (
                    ...
                    >
                      <div className="flex items-center gap-6 flex-1">
                        <div className="w-12 h-12 rounded-xl bg-slate-800 border border-slate-700 overflow-hidden flex-shrink-0 flex items-center justify-center font-black text-amber-500 shadow-inner">
                          {req.student?.avatar ? (
                            <img
                            ...
                            (req.student?.firstName || 'U').charAt(0)
                          )}
                        </div>
            ...
            <div className="p-10 bg-slate-950/50 border-t border-slate-800 flex gap-4">
              {['In Progress', 'Resolved', 'Rejected'].map((status) => (
                <button
                  ...
                  className={`flex-1 py-5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-3 ${selectedRequest.status === status ? 'bg-slate-800 text-slate-500 cursor-not-allowed border border-slate-700' : status === 'Resolved' ? 'bg-teal-600 hover:bg-teal-500 text-white shadow-lg shadow-teal-500/10' : status === 'Rejected' ? 'bg-red-950/50 hover:bg-red-900/50 text-red-500 border border-red-500/20' : 'bg-white hover:bg-slate-200 text-slate-950 shadow-lg'}`}
                >
