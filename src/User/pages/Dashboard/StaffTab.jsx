...
const StaffTab = () => {
  ...
  const staff = [
    {
      ...
      info: 'Contact for book issues, study space availability, and research help within the core knowledge node.',
    },
    {
      id: 2,
      name: 'Maria Garcia',
      role: 'IT Support',
      dept: 'Tech Hub',
      info: 'Contact for Wi-Fi access, portal login issues, and lab assistance in the digital stream.',
    },
  ];
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      toast.success('Support request synchronized successfully.');
      ...
      >
          ...
          <div className="flex gap-10 items-center">
            ...
            >
              ...
              <p className="text-blue-950/30 font-bold uppercase tracking-[0.2em] text-[10px] flex items-center gap-2 mt-5">
                <Headphones size={16} className="text-teal-500 animate-pulse" /> {selectedItem.role}
              </p>
            </motion.div>
          </div>
          <motion.div
        ...
        <form onSubmit={handleSubmit} className="space-y-16 relative z-10">
          <motion.div
            ...
            className="bg-blue-50/50 p-12 rounded-[40px] border border-teal-50 relative shadow-inner"
          >
            <div className="absolute -top-4 -left-4 bg-white p-4 rounded-2xl border border-teal-50 shadow-2xl shadow-teal-100/50">
              <Info size={22} className="text-teal-500" />
            </div>
            <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-blue-950/20 mb-6 flex items-center gap-2">
              Unit Protocol Specs
            </h4>
            <p className="text-blue-950/60 leading-relaxed font-medium text-lg border-l-4 border-teal-400 pl-8">
              {selectedItem.info}
            </p>
          </motion.div>
          ...
          <motion.div
            ...
            className="space-y-8"
          >
            <label className="text-[10px] font-bold text-blue-950/20 uppercase tracking-[0.3em] ml-2 flex items-center gap-3">
              <Radio size={14} className="text-teal-500" /> Request Specification
            </label>
            <textarea
              required
              ...
              placeholder="Describe the assistance required from this unit..."
            ></textarea>
            <motion.button
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-6 text-blue-950/30 font-bold uppercase tracking-[0.2em]">
                  <span className="text-[10px] flex items-center gap-2">
                    <Headphones size={14} className="text-teal-500" /> {s.role}
                  </span>
                  <div className="w-1.5 h-1.5 bg-teal-400 rounded-full"></div>
                  <span className="text-[10px] flex items-center gap-2">
                    <Building size={14} className="text-teal-500" /> {s.dept}
                  </span>
                </div>
              </div>
            </div>
